...
export default function AdminSidebar({ mobileOpen, setMobileOpen }) {
  ...
  return (
    <>
      ...
      <aside
        className={cn(
          'fixed top-0 bottom-0 left-0 z-50 w-72 bg-[#121921] text-ivory border-r border-gold/20 flex flex-col justify-between transition-transform duration-300 ease-in-out lg:translate-x-0 lg:static',
        ...
        <div>
          ...
          <div className="p-6 border-b border-ivory/10 flex items-center justify-between">
            <div className="flex items-center gap-3">
              <div className="h-10 w-10 rounded-full border border-gold bg-gold/10 flex items-center justify-center text-gold">
          ...
          {/* Navigation Items */}
          <nav className="p-4 space-y-6 font-sans text-body-xs overflow-y-auto max-h-[calc(100vh-140px)]">
                ...
                {group.items.map((item) => {
                  ...
                  return (
                    <Link
                      ...
                      onClick={() => setMobileOpen(false)}
                      className={cn(
                        'flex items-center gap-3 px-3.5 py-2.5 rounded-luxe transition-all duration-300',
                        isActive
                          ? 'bg-gold text-espresso font-semibold shadow-gold-sm'
